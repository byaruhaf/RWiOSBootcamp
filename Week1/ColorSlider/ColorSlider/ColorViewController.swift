...
extension ColorViewController {
    ...
    fileprivate func getUserSavedColors() {
        ...
        guard let currentHex =  defaults.string(forKey: "currentHex") else { return }
        self.hexLabel.isHidden = false
        self.hexLabel.text = currentHex
    ...
    fileprivate func saveUserColors() {
        ...
        // current value of slider 2
        defaults.set(sliderTwo.value, forKey: "sliderTwo")
        // current value of slider 3
        defaults.set(sliderThree.value, forKey: "sliderThree")
        // current value of Hex
        defaults.set(currentHex, forKey: "currentHex")
    }
    ...
    fileprivate func clearUserSavedColors() {
        ...
        // remove saved  value for slider 2
        defaults.removeObject(forKey: "sliderTwo")
        // remove saved  value for slider 3
        defaults.removeObject(forKey: "sliderThree")
        // remove saved  value for currentHex
        defaults.removeObject(forKey: "currentHex")
    }

    fileprivate func getHexValue() -> String? {
        switch currentMode {
        case .isRGB:
            return valueToHex()
        case .isP3RGB:
            return valueToHex()
        case .isHSB:
            return calculateColor().toHex()?.uppercased()
        }
    }
